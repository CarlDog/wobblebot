...
- **ADR-021/ADR-037 alerting-fidelity gaps: DMS-alert reset bug,
  ...
  unexplained external cancel — it now cites the live DMS-failure
  streak to pick the honest framing, with severity and the HOLD itself
  unchanged; (3) a held symbol got exactly one notification ever, with
  no reminder it was STILL held — production went ~18h with 5 symbols
  held and only the initial alerts to notice by. A new aggregate
  "still held" reminder now fires on a configurable cadence
  (`live.held_reminder_seconds`, default 4h) while any symbol remains
  held. Full detail in `docs/planning/roadmap.md`'s Stage 8.4.E entry.
  15 new regression tests, including two verified failing against the
  pre-fix code and passing against the fix.
