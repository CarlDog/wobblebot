...
| ------------------------------ | ----------------------------- | ----- |
| `IMAGE_TAG`                    | all services                  | Image tag to deploy. Pin to something immutable for guaranteed pulls: `sha-<short>` (e.g. `sha-de318e2`) for any build, or a release version (e.g. `2.0.0`) at a release — the latter makes "what's running" legible. Avoid `:latest` / `:main` / `:X.Y`; they are mutable and Docker often skips re-pulling them. Defaults to `main` if unset. |
...
Then open `https://wobblebot.your-nas` (DSM reverse proxy fronts
