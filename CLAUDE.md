...
**Stage 4.4 closed 2026-05-15** (four slices). `KrakenAdapter.withdraw()` implemented against `/0/private/Withdraw` (was a Phase 1.2 stub). `HarvesterConfig` grew `api_key_env_var` / `api_secret_env_var` / `withdrawal_destinations` (asset → Kraken Pro label dict) / `proposal_max_age_hours`. `cli/harvest` switched to the Harvester key (Withdraw scope). New `transfer_results` SQLite table + `compute_today_total_withdrawn_usd` helper (real rolling-24h day-cap, replacing the `Decimal("0")` placeholder). **`cli/harvest --execute <proposal-id>`** is the operator-approval gate — six defense layers (`enabled=True`; proposal exists; not stale; destination label resolves; current balance ≥ amount; day-cap headroom) before `adapter.withdraw()` ever runs. New `tools/show_transfers.py` inspector mirrors `tools/show_proposals.py`. 888 unit tests pass; mypy clean (60 src files); pylint 10.00/10. **No real withdrawal happened during slice tests** — the first live $1 ACH withdrawal is the operator-triggered first execution, scheduled separately.

**Next:** Phase 4.5 — Phase 4 integration check + Phase 4 summary doc (mirrors `phase-3-summary.md`). Requires a real first withdrawal (operator's $1 ACH plan against the "360 Performance Savings" destination once balance enters surplus band). Operator's current $99.92 balance is below the $200 floor — to exercise the full pipeline live, they'd need to either deposit USD to push into surplus OR adjust `surplus_threshold_usd` to make their current state classify as surplus. Per ADR-003 the Harvester is the only module with transfer authority; per ADR-004 it uses Kraken's withdrawal API via `ExchangePort` (no separate banking adapter). The Phase 4 trade-vs-Harvester key separation is the load-bearing safety property the phase exists to demonstrate. Cloud-provider adapters (anthropic / openai / google) remain placeholder slots in `_build_advisor` — they raise "not implemented" today and need their own slice when the operator wants cloud experts in an MoE lineup. Not blocking Phase 4. Stage 5.3.5 (Background Maintenance Worker) is on the roadmap but deferred to Phase 5 — the unified `schedules:` block from Stage 3.3 Slice C.0 is the contract the maintenance worker will plug into. CryptoCompare 90-day evaluation due 2026-08-13.
