...
**Phase 2 closed 2026-05-14; Phase 3 Stages 3.0 (Observer & Shadow), 3.1 (Data Collector & Metrics v2), 3.2 (Advisor Port + single-LLM Ollama), 3.2.5 (News Ingestion), and 3.3 (Passive Advisory Workflow) all closed by 2026-05-15.** Closing summary for Phase 2 at `docs/planning/phase-2-summary.md`. Total real-money cost across both live verifications: **$0.08** (the $0.08 first-trade test in `tools/first_real_trade.py` + the $0.00 multi-coin grid run in Stage 2.5). Stage 3.3 added `cli/advise` + `AdvisorSuggestion` storage + the unified `schedules:` config block — no new live-money risk per ADR-002 (LLM advisory-only). **Eight** operator entry points work end-to-end (Stage 3.3 added `cli/advise`):
...
675 unit tests pass by default (up from 619 after Stage 3.3 added 56 across the AdvisorSuggestion domain + storage, SummaryBuilder, schedules parser, cli/advise wiring, and tools/show_suggestions); 21 integration tests (5 Kraken API drift + 3 live read + 2 simulator + 2 grid e2e + 9 live trading) on opt-in. mypy clean (52 src files), black/isort clean, pylint **10.00/10** on `src/`. New runtime dep added in 3.2.5: `feedparser`.
...
**Next:** Phase 3 stages 3.4a → 3.5 per the roadmap (MoE adapter with cloud-LLM experts → bounded auto-tuning → integration check). Per ADR-002 + ADR-007 the advisor cannot execute, so no new live-money risk over Phase 2. Stage 5.3.5 (Background Maintenance Worker) is on the roadmap but deferred to Phase 5 — the unified `schedules:` block from Slice C.0 is the contract the maintenance worker will plug into. CryptoCompare 90-day evaluation due 2026-08-13.
