...
**Phase 2 closed 2026-05-14; Phase 3 Stages 3.0 (Observer & Shadow), 3.1 (Data Collector & Metrics v2), 3.2 (Advisor Port + single-LLM Ollama), 3.2.5 (News Ingestion), 3.3 (Passive Advisory Workflow), 3.4a (Mixture of Experts), and 3.4b (Bounded Auto-Tuning Gate) all closed by 2026-05-15.** Closing summary for Phase 2 at `docs/planning/phase-2-summary.md`. Total real-money cost across both live verifications: **$0.08** (the $0.08 first-trade test in `tools/first_real_trade.py` + the $0.00 multi-coin grid run in Stage 2.5). Stage 3.4b added `evaluate_auto_apply` gate + `cli/apply` dry-run + `--commit` settings.yml rewriter (ruamel.yaml) + `AppliedSuggestion` audit table — gate defaults off, news-role suggestions never auto-apply per ADR-007. **Nine** operator entry points work end-to-end (Stage 3.4b added `cli/apply`):
...
**Next:** Phase 3 stage 3.5 (Phase 3 integration check) — advisor-in-the-loop demo with MoE producing aggregated suggestions and the operator optionally auto-applying bounded ones via `cli/apply --commit`. All the pieces ship; 3.5 wires them together for a longer-running soak. Per ADR-002 + ADR-007 the advisor still cannot execute trades, so no new live-money risk over Phase 2 (gate mutates config; engine reads config at startup). Cloud-provider adapters (anthropic / openai / google) are placeholder slots in `_build_advisor` — they raise "not implemented" today and need their own slice when the operator wants cloud experts in an MoE lineup. Stage 5.3.5 (Background Maintenance Worker) is on the roadmap but deferred to Phase 5 — the unified `schedules:` block from Stage 3.3 Slice C.0 is the contract the maintenance worker will plug into. CryptoCompare 90-day evaluation due 2026-08-13.
