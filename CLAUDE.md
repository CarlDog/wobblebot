...
- `python -m wobblebot.cli.harvest` — Stage 4.2 read-only treasury monitor. Polls Kraken USD balance on `schedules.harvest`, runs `propose_transfer()` against operator's `HarvesterConfig` thresholds, logs "HYPOTHETICAL proposal (no money moved)" lines. No transfers, no DB writes; uses the read-only `KRAKEN_API_KEY` (the Harvester key with Withdraw scope isn't needed until Stage 4.4).
...
**Stage 4.3 closed 2026-05-15.** Every non-None proposal now persists to a new `transfer_proposals` SQLite table for operator review. `TransferProposal` gained `created_at`; `StoragePort` grew `save_transfer_proposal` / `get_transfer_proposals`; `HarvestConfig` grew a `db` field; `cli/harvest` persists every proposal on every tick. Storage write failures log + continue (missing audit row < missing every subsequent tick). New `tools/show_proposals.py` mirrors `tools/show_suggestions.py` shape. Persistence is independent of `HarvesterConfig.enabled` — that flag gates execution at 4.4+, not the forensic record. 853 unit tests pass; mypy clean (60 src files); pylint 10.00/10.

**Next:** Phase 4.4 — active-mode withdrawals. Needs the Harvester Kraken key with Withdraw scope (KRAKEN_HARVESTER_API_KEY in .env) and pre-registered Kraken withdrawal destinations via the Kraken Pro UI. `cli/apply`-style operator-in-the-loop pattern: `cli/harvest --execute <proposal-id>` (or similar) reads the persisted proposal, validates it's still timely + within bounds, calls Kraken withdrawal API via ExchangePort, persists a TransferResult row. Per ADR-003 the Harvester is the only module with transfer authority; per ADR-004 it uses Kraken's withdrawal API via `ExchangePort` (no separate banking adapter). The Phase 4 trade-vs-Harvester key separation is the load-bearing safety property the phase exists to demonstrate. Cloud-provider adapters (anthropic / openai / google) remain placeholder slots in `_build_advisor` — they raise "not implemented" today and need their own slice when the operator wants cloud experts in an MoE lineup. Not blocking Phase 4. Stage 5.3.5 (Background Maintenance Worker) is on the roadmap but deferred to Phase 5 — the unified `schedules:` block from Stage 3.3 Slice C.0 is the contract the maintenance worker will plug into. CryptoCompare 90-day evaluation due 2026-08-13.
