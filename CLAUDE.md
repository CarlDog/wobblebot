...
**Next:** Phase 6 — **Cloud LLM Integration**. Operator-selectable cloud LLM providers for both the operator-assistant role (Phase 5 added) and the MoE trading-advisor roles (Phase 3 placeholder slots). `AssistantPort` is provider-neutral by construction; Phase 6 extends `AssistantLLMConfig.provider` from `Literal["ollama"]` to include `anthropic` / `openai` / `google` and adds the corresponding adapter classes alongside cloud trading-advisor adapters in `_build_advisor`. Likely adds an `llm_calls` cost-tracking table since cloud APIs charge per request. **Phase 7 Web UI / Dashboard** and **Phase 8 Hardening & v1.0 Release** (reliability + background maintenance worker + performance tuning + v1.0 soak) follow per the Phase 5 kickoff reorg. Per ADR-003 the Harvester remains the only module with transfer authority; per ADR-004 it uses Kraken's withdrawal API via `ExchangePort` (no separate banking adapter). CryptoCompare 90-day evaluation still due 2026-08-13 (falls inside Phase 6).
