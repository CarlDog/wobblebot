...
**Next:** Stage 3.6 (operational polish, pre-Phase 4) — two small slices to remove friction before Phase 4 lands the Harvester loop. **Slice 3.6a**: `max_runtime_minutes` becomes `Optional[float]` with `None` meaning "no cap" (Pydantic `gt=0` currently rejects `0`; the loop check at `cli/live.py:193` would exit on tick 1 anyway). Applies to both `LiveConfig` and `ShadowConfig`. **Slice 3.6b**: `cli/advise` grows multi-symbol (`AdviseConfig.symbols: list[Symbol]`) with per-symbol-isolated LLM calls — the daemon iterates serial per tick so each cycle's `PerformanceSummary` carries one coin's context (no cross-contamination, operator-ratified design call). Both ~1 evening total.

After 3.6: Phase 4 — Harvester & Treasury Management. Per ADR-003 the Harvester is the only module with transfer authority; per ADR-004 it uses Kraken's withdrawal API via `ExchangePort` (no separate banking adapter). The Phase 4 trade-vs-Harvester key separation is the load-bearing safety property the phase exists to demonstrate. Cloud-provider adapters (anthropic / openai / google) remain placeholder slots in `_build_advisor` — they raise "not implemented" today and need their own slice when the operator wants cloud experts in an MoE lineup. Not blocking Phase 4. Stage 5.3.5 (Background Maintenance Worker) is on the roadmap but deferred to Phase 5 — the unified `schedules:` block from Stage 3.3 Slice C.0 is the contract the maintenance worker will plug into. CryptoCompare 90-day evaluation due 2026-08-13.
