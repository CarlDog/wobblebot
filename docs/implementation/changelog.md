...
- Phase 1.5 — End-to-end integration check. `wobblebot.services.simulator.run_buy_dip_sell_rebound_cycle` wires `ExchangePort` + `StoragePort` to execute a hard-coded buy-low / sell-high cycle against a scripted price walk, persisting every order transition and trade plus a final balance snapshot. `python -m wobblebot.cli.simulate` is the operator-facing entry point (writes SQLite to ``wobblebot-sim.db`` by default, configurable via `--db`; log format via `--log-format plain|json`). Integration test in `tests/services/test_simulator.py` runs against `:memory:` SQLite and verifies trades, orders, and balance math end-to-end. **Phase 1 complete.**
- Pydantic mypy plugin enabled in `pyproject.toml` so mypy correctly recognizes Pydantic `Field(...)` defaults. Without it, `Order(...)` construction in src/ would fail with spurious "Missing named argument" errors.
