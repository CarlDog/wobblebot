...
- **2026-09-01 — logging redaction at the formatter boundary.** A CodeQL wave
  found ten Harvester log flows that could carry upstream exception text. The
  Harvester messages already name environment variables rather than secret
  values, but a generic exception can echo a request credential. Both logging
  formats now redact the process's configured API-key, secret, token, password,
  and webhook values from interpolated messages, structured extras, and plain
  or JSON exception text. Twenty formatter tests and 38 Harvester regressions
  pass; diagnostic variable names and error types remain visible.
   ...
   **Post-build backtest (2026-05-29) — the vol→spacing premise is NOT validated.**
   ...
   - `tools/grid_backtest.py`). **Actionable:** consider widening the live grid toward
     ...
     **Update (multi-coin + chop tested — 5 coins, 2024/2025 + per-coin chop windows):**
     ...
     _genuine_ chop (alts +8% to +28%; the 2025 rejection was a crash-year confound),
     ...
     the per-symbol/per-regime _spacing_ (calibration), trend/regime sets _win-vs-lose_
   ...
   - an arbiter ruled the verdict `verdict-mostly-holds-with-revisions` (5 of 6
     skeptic biases run _against_ the grid; both deliberate break-paths failed; §10 of
...
7. **Stage 8.6 – Advisor HARDENING + Grid Widen** ✅ 2026-05-30 (kickoff 2026-05-29,
   ...
   **What shipped:**
   - **Slice C — grid widen** (`a1b39c4`): live BTC `grid.default.spacing_percentage`
     1.0 → 3.0% (the least-bad _static default_; exposure unchanged at $60 = 3+3 × $10;
   ...
   - **Slice A — curve recalibration: DEFERRED to the Oracle/regime track** (no code).
     ...
     3%, never tighten" would bake in a false absolute — a tight grid _chosen in chop and
     pulled before the trend_ genuinely works (proven live + oracle +164.6%). The advisor
   ...
   - **Slice D — close-out:** ratified **ADR-019** (advisor purpose: regime reader +
     ...
     **Parked (not deleted), on the Oracle/MoE research track** (synthesis §4): the
     ...
     Two levers: (1) volatility → _base_ spacing calibration (recalibrate the curve to
...
_shipped_ items are receipted here.
...
3. **Cache-aware LLM cost accounting** ✅ 2026-08-02 — **ADR-033**, from the prompt-caching
   ...
   ADR-014 ledger. Actively _enabling_ Anthropic `cache_control` is **deferred with
...
5. **P3 — ops/observability/UX ✅ COMPLETE (2026-08-08 → 2026-08-10), and P4 — advisor feedback ✅ BUILDABLE SCOPE COMPLETE (2026-08-16 → 2026-08-17).**
   _(Header corrected 2026-08-28: it read "P3 IN PROGRESS" long after both phases closed. The completion receipts were always in this item's body — "P3 COMPLETE (buildable scope)" below, then the P4 kickoff and P4.1–P4.7 receipts — but the line a reader scans said otherwise. P4.6 Historian remains the one open P4 item, gated on the Q2-dump canonical scoring run.)_
   ...
   **Slice 2 — alert-quality follow-up** ✅ **2026-08-08** (both findings from
   ...
   withdrawal _execution_ — the daemon legitimately runs proposals-only with it off);
...
deferred)` — BTC had been parked offside since the 16:42 UTC restart (hence
   ...
   **Slice 13 — modal-card action flow** ✅ **2026-08-09** (the operator's
   ...
   all-verbs test. Test count 2882 → 2886. _Follow-up (operator-caught,
   same day):_ closing the modal mid-watch (Close/Cancel/Escape before the
   ...
   **Slice 19 — notifications server-side read-state + Acknowledge + deep
   ...
   clearing the dot on the desktop left the phone dotted, and merely _opening_
   ...
   **Bug caught by rendering it, not by the tests:** the summary first summed
   the per-row "net USD" column verbatim and called the result _net_. That
   ...
   **The design note's three open questions, answered:** window = 14d hourly;
   ...
   in a dead market is idle capital and the right response may be _pause_, not
   ...
   **Same-slice fix — `status.py` split at the 1000-line gate.** The
   ...
   loaders, and the viability reader — one cohesive question (_should the
   operator re-anchor, and is it worth it?_) with its own thresholds and
   ...
   **Slice 23 — design-review leaves** ✅ **2026-08-10**: the five remaining
   ...
   poll forever, at which point the highlight stops meaning _this just
   happened_; `prefers-reduced-motion` degrades to a static tint so the
   ...
   **Post-P3 — Ollama schema-constrained generation** ✅ **2026-08-10** (not a
   ...
   string `format: "json"`, which guarantees the body _parses_ and says nothing
   about its _shape_. That gap was live, not theoretical: a MoE panel run had
   ...
   **⚠️ Recorded honestly: the fix trades a LOUD failure for a QUIET one.** The
   re-run completed instead of erroring, but `phi4:14b-q8_0` (quant _and_
   arbitrator) then produced schema-_valid_ token salad —
   ...
   drawdown and cap headroom — so the panel _can_ do this. Two caveats before
   ...
   **Quant seat — DECIDED: cloud `gpt-5-mini`.** Core battery (12 fixtures,
   ...
   _partly rule-following_: when a secondary signal should override volatility, it
   ...
   **⚠️ Cost-gate caveat, recorded in `.env.example`** — _this paragraph
   ...
   call)._ `get_price_point()` **raises** `PricingLookupError` on an unmodeled
   ...
   **↳ ROOT-CAUSED 2026-08-10, same session — and it is NOT a `quant.md` gap.**
   ...
   Three of the five are cases the deterministic guards handle _before_ the LLM
   ...
   **Seat decisions: NO SWITCH** — deciding evidence is the freejudge run
   ...
   failing `heldout_drawdown_overrides_calm` _less badly_ than gpt-5-mini (held
   ...
   **NO SWITCH — `gpt-5-mini` holds the quant seat.** Sonnet 5's clean UNSAFE
   ...
   2026-08-31 moving it _further_ out of band ($2/$10 → $3/$15).
...
is deprecated for this model`. The first roster attempt scored sonnet-5 and
   ...
   the advisor escalation seat _or_ the Discord operator assistant. Fixed with
   ...
   **The cadence question that started it, answered:** churn between 4h
   ...
   changes. Reduction is nearly free — _which is the reason not to do it_:
   ...
   **A real-money production bug, found by the operator from the webui.**
   A BTC BUY filled on Kraken 2026-08-11 18:33 UTC _while the symbol was
   paused_ and the engine never saw it: `GridEngine.step` returned
   ...
   **Harvester ran in production for the first time.** The daemon had
   ...
   three wrong hypotheses before landing: the env var held a _valid_
   ...
   **Queued, deliberately undeployed:** `b9ec699` annotates the
   ...
   _(Superseded 08-16: the repo-quality batch deployed as `sha-f6f3f04`
   carrying both.)_
   ...
   **Re-anchor banner execution feedback** ✅ **2026-08-17** (web UX
   ...
   _recommendation_ to the honest levers (snooze / pause / wait for
   ...
   **Twin fix, same day:** `cli.live._cancel_all_open`'s shutdown-cancel
   ...
   left `status='open'`, so it surfaces as storage-only on the _next_
...
6. **`2.0.0` RELEASED ✅ 2026-08-28 — tagged, published, and deployed.**
   ...
   **$0.00** — no live trading changes, only deployment and config-boundary work.
   ...
   - **ADR-041 — the deployment enforces the capability matrix** (PR #113). One
     `x-wobblebot-defaults` anchor had been injecting _every_ credential into
     _all nine_ services: `web`, `news`, `advise`, `operator`, `maintenance` and
     ...
     ADR-003's fragmentation was enforced in the Python and merely _assumed_ at
     ...
     `tests/deployment/test_compose_capability_matrix.py` (an _extra_ credential
   ...
   - **The v1.0 → 2.0 upgrade-survivor gate** — builds its fixture from the real
     ...
     **It found a real defect immediately:** a v1.0 `settings.yml` keeps
     `safety.emergency_stop` and 2.0 ignored it _silently_ (no config model sets
   ...
   - **Two CI/release-path defects, both found by checking rather than trusting.**
     (a) The upgrade gate was _silently skipping_ in CI — `actions/checkout` is
...
10. **`2.0.2` RELEASED ✅ 2026-09-03 — tagged and published; deploy is a separate
    ...
    **Deployed 2026-09-03 18:58 UTC** on operator instruction: `IMAGE_TAG`
    `2.0.1` → `2.0.2` via the stack-env update on file-based stack 158 (image
    re-pulled; all 8 containers recreated at 18:58:12 UTC). Verified at the
    container: `org.opencontainers.image.revision` = `76819c6…`,
    `org.opencontainers.image.version` = `2.0.2` on `wobblebot-live`; every
    container on `ghcr.io/carldog/wobblebot:2.0.2`. cli/live closed the 2.0.1
    session cleanly (exit 0, 543 ticks, 7 open orders cancelled, 0 failures,
    session PnL +$0.03) and re-opened at 18:58:27 with live TradeVolume fee
...
14. **Post-2.0.4 backlog Groups 1-3, then `2.0.5`** ✅ 2026-09-04.
    ...
    Ceremony receipts: all four merges — #134 `14c4823` (Group 1, operator
    legibility), #135 `4f4cda2` (Group 2, starvation), #136 `783e192`
    (Group 3, persisted per-symbol state), #137 `b4d88ce` (Group 0, docs).
    This list first named #135–#137 only, omitting the very group the
    item's prose leads with — corrected 2026-09-04 by the release-close
    audit; release commit `a9a5dcb` with main CI green; annotated tag
