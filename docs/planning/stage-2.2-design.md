...
| **2.2.3 — Grid engine service** | `services/grid_engine.py`. Wires the pure logic to `ExchangePort` (read price, place orders) + `StoragePort` (persist orders/trades/grid slots). Exposes `GridEngine.step()` that advances one tick: read current price → reconcile open orders → check fills → place counters and missing levels. | ~2-3 hours (largest) |
...
| **2.2.6 — CLI wrapper** *(optional)* | `cli/paper` (or extend `cli/simulate`) to run the engine indefinitely against the mock with operator-visible logging. Honestly **defer this to Stage 2.3** — that's when the operator wants to actually watch it run. | — |
