...
### Regime detector — classify market state without predicting price

**What:** new ``services/regime_detector.py`` that classifies the
current market state into one of a small set of named regimes:
``bull_trending`` / ``bear_trending`` / ``high_volatility_chop`` /
``mean_reverting`` / ``unknown``. Output is a ``RegimeClassification``
value object carrying the kind + a confidence score + the window of
data the classification was computed over.

**Why high value:** several v1.1 candidates depend on having a
shared, deterministic notion of "what kind of market are we in
right now": regime-aware grid modes (above), confidence-driven
grid extension, operator-initiated re-anchor (could surface "re-
anchor recommended; regime=bear_trending" context), the anomaly
detector ("anomaly OR known regime shift?"), and the outcome
evaluator's per-regime success-rate slicing. Centralising the
classifier means all consumers agree on what regime is current
rather than each reinventing thresholds.

Note: classification is **not** prediction. The detector says
"based on the last N candles, this looks like X"; it does NOT say
"the next M candles will be Y." Consumers that take action based
on the classification still need their own conviction layer (e.g.
the regime-aware grid modes entry's hysteresis requirement before
switching shapes, the confidence-driven extension's outcome-
evaluator gate).

**Why ADR-002-respecting:** the detector is a pure function over
price history with deterministic output. No LLM in the classifier
path. The advisor remains advisory; the detector is operational
ground-truth that the advisor *and* the engine can consume.

**Implementation candidates (three to evaluate during prototype):**

1. **Simple rolling-window thresholds.** Compute realized
   volatility + linear-regression slope over a 24h/7d/30d window;
   classify by quadrant. Pros: deterministic, debuggable, fast.
   Cons: hard thresholds whipsaw at boundaries.
2. **HMM (Hidden Markov Model).** Train offline on historical
   BTC data; persist transition + emission probabilities; classify
   live by forward algorithm. Pros: smooth boundary behavior;
   captures regime persistence. Cons: training dataset + serial-
   correlation handling; "stationary regime" assumption violated
   across major market structure shifts.
3. **LLM-arbitrated over numeric features.** Compute the
   feature set as in #1 + recent news-role outputs; pass a
   structured prompt to the local LLM; require schema-validated
   classification output. Pros: incorporates qualitative signal.
   Cons: cost, latency, ADR-002 calibration question (does the
   classification count as an "advisory" output? probably yes,
   which means the auto-apply-style gate would need to cover it).

Prototype phase: implement all three behind a single
``RegimeDetectorPort`` interface; shadow-run for 60-90d alongside
the live engine without affecting trading; compare classifications
against operator's eye-of-the-needle judgments. Whichever has the
best agreement + lowest whipsaw becomes the v1.1 default; the
others can stay available as alternatives.

**Why deferred:** prerequisite work (60-90d of price + news +
operator-classified data) hasn't accumulated yet; without that
calibration set, the detector's thresholds are guesses. Also: the
v1.1 features that *consume* the detector (regime-aware grid
modes, anomaly detector, outcome evaluator) are themselves
deferred — there's no point shipping the detector without a
consumer, and there's no consumer that's been validated against
soak data either.

**Trigger:** post-v1.0, after 60-90d of soak has accumulated the
calibration dataset; pairs with regime-aware grid modes + the
outcome evaluator + the anomaly detector as the "adaptive grid"
feature group.
...
implementing):** ADR-004 rejected a separate banking adapter
because Kraken's REST API is exchange→bank only for fiat. The
withdrawal endpoint exists; the corresponding deposit-pull
endpoint does *not* — fiat deposits today are operator-initiated
ACH pushes from the bank side, not API-initiable from Kraken's
side (this is what the Stage 4.5 audit caught in the seventh
defense layer). Three potential paths to evaluate:

1. **Kraken API gains a deposit-pull endpoint.** Already tracked
   as a v1.x candidate (this entry, previously a placeholder).
   Trigger: Kraken announces a deposit API. Cleanest path:
   adapter mirrors withdraw.
2. **Plaid integration via Kraken's funding partner.** If
   Kraken's Plaid-mediated instant deposit flow is invokable via
   an API key with the right scope, the Harvester could trigger
   the same flow programmatically. Needs investigation of
   Kraken's actual API surface beyond the public REST docs (some
   funding endpoints are behind the WebUI).
3. **Scheduled ACH push from bank side + Harvester observation.**
   ...
   *observer* — it watches for the expected deposit arrival on
   Kraken, surfaces an alert if it doesn't arrive on schedule,
   and adjusts the next top-up schedule. This path doesn't
   require any Kraken API capability beyond what we already
   have. Less elegant but doable today.

Path 3 is the realistic v1.1 default unless investigation surfaces
that Path 1 or 2 is genuinely available.
...
- Regime detector — pure-function classifier (`bull_trending` / `bear_trending` / `high_volatility_chop` / `mean_reverting` / `unknown`); prerequisite for adaptive-grid feature group; not prediction → Group 2
...
- Harvester top-up deposits — bank→exchange direction; savings-account-anchored bounded-failure-mode; technical-feasibility investigation gates implementation path (Kraken deposit API / Plaid / scheduled ACH observation) → Group 2
