...
### Stale-order alerting + auto-cancellation policy

**What:** the status card now shows per-order age via the
`order_ages` field on `StatusSnapshot` (soak Day 4 add). Two
operator-facing extensions remain:

1. **Visual alerting:** color the row (or just the age cell) when
   the order age crosses a threshold — e.g. amber > 24h, red > 72h.
   Pure CSS + a per-row conditional in the template. Respects
   ADR-006 (engine still stays parked when offside); just makes
   the staleness visible.
2. **Auto-cancellation policy:** engine-side cancellation of
   orders older than N days. Two technical paths considered and
   rejected for v1.0:
   - **Kraken `expiretm`** (engine sets order expiration at
     AddOrder time). Pro: zero runtime cost; Kraken does the
     work. Con: order silently disappears from the book; engine
     doesn't know until reconciler runs, so grid silently
     shrinks with no replacement logic.
   - **Wobblebot-side stale cancel** (`cli/live` checks each
     tick, cancels orders > N days). Pro: under engine control.
     Con: same "grid shrinks" issue without paired re-placement
     logic.
   Both options would interact badly with ADR-006's "engine stays
   parked when offside" decision and the existing
   operator-initiated re-anchor v1.1 candidate.

**Why deferred:** alerting is template-only and could ship in a
soak-period commit, but the *real* question — "what should
happen when a BUY ladder has been parked 4 days and the market
trended away?" — is the **operator-initiated re-anchor** (already
a v1.1 candidate from soak Day 2-3). Alerting tells the operator
when to invoke that; auto-cancellation would have the engine
make the decision, which conflicts with the existing safety
posture.

**How (alerting only):** add a `.row-stale-warn` / `.row-stale-red`
CSS class set conditionally in the template based on
`snapshot.order_ages.get(o.id|string, 0)` against thresholds
(e.g. 86400 = 1d, 259200 = 3d). Tune defaults during soak as
the operator observes which ages actually matter.

**Trigger:** operator surfacing during soak Day 4 nightly check
that the new age column is informative but they want the rows
themselves to alert when ages cross thresholds. The
auto-cancellation discussion has its own trigger: a soak-period
incident where stale orders sit so long the operator finds them
operationally distracting AND the
operator-initiated-re-anchor mechanism turns out to be too
manual.
...
**Trigger:** any further soak-period bounce that exhibits the
"exit-clean log fires but process doesn't die" pattern. With the
soak running 4+ weeks and the operator bouncing for code
updates, this'll likely surface again. Two more instances and
the case for v1.1.A treatment becomes clear.
...
- Stale-order alerting + auto-cancellation policy → Group 2
