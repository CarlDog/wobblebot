...
**Leading candidate — Raspberry Pi on UPS (operator has one
sitting around).** Pi 5 with 8 GB RAM handles 7 Python daemons +
Ollama (with a smaller-model loadout, ~7B-quantized) comfortably.
Headless setup, Wireguard or Tailscale for operator access, the
existing cli/web reverse-proxy posture (ADR-016) extends cleanly
to LAN-only behind the Pi. Cost: $0 ongoing (operator already
owns hardware; one ~$100 UPS).

**Alternative — cloud VPS.** DigitalOcean / Hetzner / Fly.io
basic droplet runs the stack for ~$5-12/month. Pros: removes
power + ISP failure modes entirely; Kraken latency typically
better than residential. Cons: (a) shifts secrets onto a 24/7
attack surface (Kraken keys, Discord token, web session secret);
(b) Kraken API key IP allowlists must be updated to the cloud
IP and re-allowlisted on rotation; (c) needs an offsite backup
strategy for SQLite DBs (cli/maintenance currently writes local
files); (d) adds CI/CD complexity if updates aren't done by SSH.

**Counter-argument worth keeping in mind:** the Day-2 outage
**exposed a real engine defect** (`e2b6cfc` finally-block fix).
A cloud host would have masked it indefinitely. There's value
in keeping the soak environment representative of "things will
fail." Argues for not migrating *during* soak, regardless of
which destination is picked.

**Implementation (Pi path):** package the 7 daemons as a
docker-compose stack (or as systemd units; either works on
Pi OS). Each daemon gets its own service definition; shared
volume for `data/` so SQLite paths line up. Secrets via
docker-compose's env_file or systemd's
EnvironmentFile=, sourced from a non-root-readable file. Kraken
key IP-allowlists updated to the Pi's WAN IP (or to a Wireguard
tunnel endpoint). Operator accesses via Tailscale to the cli/web
UI; no port-forwarding on the home router needed.

**Implementation (cloud path):** same docker-compose stack but
on a VPS, with secrets via the platform's secret manager
(Doppler, 1Password Connect, DO App Platform secrets). Offsite
backup via the existing `cli/maintenance` backup hooks
pointed at S3 / B2 / rclone-to-NAS. Reserved static IP for the
Kraken allowlist. cli/web behind Caddy/Traefik with Let's
Encrypt TLS.
...
**Trigger:** post-v1.0 tag. The Pi candidate is the most
operationally honest first step — same architecture, different
host. If operator outgrows the Pi (multi-symbol expansion +
heavier LLM models on the advisor side), revisit cloud.
...
- Always-on hosting topology — decouple from operator laptop (Pi-on-UPS leading; cloud alternative weighed) → Group 2
