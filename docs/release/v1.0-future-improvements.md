...
**Trigger:** the operator demonstrates sustained comfort with
spot grid behavior over many months AND has researched / accepted
the asymmetric downside that liquidation introduces AND explicitly
wants leveraged exposure (which is a separate financial decision
from "I want the grid to do more"). v1.2+ candidate, not v1.1.
...
**Trigger:** explicitly NOT triggered by "I want more upside" or
"shorting sounds cool". Triggered only by: (a) margin trading
shipped first (v1.2+) and operator gained sustained comfort with
that asymmetric risk model, AND (b) operator has researched
funding rates / mark prices / ADL on the actual contracts they'd
trade, AND (c) operator has read at least one good post-mortem
of a futures grid blowing up during high-funding or fast-market
conditions. This is v1.3+ at the earliest; realistically a
research project, not a sprint.
...
- Margin trading support (v1.2+; requires new risk-model ADR) → Group 2
- Futures trading support / long-short grid variant (v1.3+ research project) → Group 2
