...
| └ Starved-symbol log noise: cap refusals repeat every retry *(found 2026-09-03, XRP/USD)* | S | low-med | | Back-off works, but each 60-tick retry re-emits the per-level `refused by safety cap` WARNINGs — ~2,400 lines/day for a symbol starved by construction (XRP: buys inventory-capped, every sell below cost). Log the breakdown once on entering starved, DEBUG per retry, a "still starved after N" WARN. Logging only. Detail in `engine.md`. |
