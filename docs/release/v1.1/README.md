...
| └ Starved-symbol log noise: cap refusals repeat every retry *(found 2026-09-03, XRP/USD)* | S | low-med | ✅ | **✅ DONE 2026-09-03.** Refusal reasons are attributed per level (`services/grid_starvation.py`), reported in the entry WARNING and an hourly summary, and the per-level cap + stale-anchor lines drop to DEBUG/INFO **only while starved**. Not logging-only after all: it repaired an unreachable heartbeat and a resume-into-silence bug, and widened `_try_place`'s return. Measured noise was ~1,152/day/symbol, not the 2,400 originally estimated. — *Original:* Back-off works, but each 60-tick retry re-emits the per-level `refused by safety cap` WARNINGs for a symbol starved by construction (XRP: buys inventory-capped, every sell below cost). Detail in `engine.md`. |
