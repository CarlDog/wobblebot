...
| Offside badge tooltip: say WHY (side, band, anchor, duration, exits) *(operator note 2026-09-03)* | S | med | | The badge's `title` is generic ("outside the grid band for N ticks"). Render one plain sentence: price is ABOVE/BELOW the top/bottom level, the anchor + spacing that define the band, the two exits (price returns / re-anchor), and how long. Band from `grid_state` via `compute_grid_levels`, never from open orders. Detail in `operator-ux.md`. |
