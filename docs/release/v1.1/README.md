...
| └ The capital report's ordermin check says "cannot be sold" about a position that can *(found 2026-09-04, tracing a wrong prediction in the 2.0.5 soak receipt)* | S | med | | The daily CAPITAL notice reads *"DOGE/USD: held 31.54235586 is below ordermin 50 (exchange balance) — the position cannot be sold as a single order"*. DOGE's actual position is **293.64029343**, of which 262.09793757 is locked in resting orders. It is entirely sellable; it is merely committed. The check reads `balance.available` at `cli/maintenance_capital.py:184`, while CLAUDE.md describes it as *"is the HELD position itself above ordermin (a position below it cannot be sold at all)"* — held and available are different quantities, and in a RUNNING grid available is structurally suppressed, because every buy fill is immediately committed to a counter-sell. Verified by arithmetic on the live tape: across the 12:30 fill, locked grew by exactly the fill size (69.95340648) and available did not move at all, so this warning cannot clear while the grid is working normally. **Two defensible fixes and the choice is an operator call:** read `total` (answering "could I exit this position", which is what the message claims) or keep `available` and reword to say the position is committed rather than unsellable. Either way the sentence as written is a confident falsehood of the same class 2.0.4 and 2.0.5 were cut to remove. Advisory path only — the capital report notifies and writes nothing. |
| └ `services/` imports `adapters/`, so the sanctioned LLM seam is bidirectional *(found 2026-09-04, release-close audit)* | S | low | | `services/llm_cloud_call.py:45` imports `OllamaJsonExtractError` / `extract_last_json_object` from `adapters/ollama.py`, and `services/simulator.py:20` imports `adapters/mock_exchange`. CLAUDE.md used to justify the LLM exception with "those helpers never import the adapters back" — false, and now corrected in place. There is still **no cycle** (`adapters/ollama.py` imports only `config/`, `domain/`, `ports/` — verified by grep), but it is cycle-free by accident, and one future import in that adapter closes the loop with nothing to catch it. Fix: `extract_last_json_object` is a pure text function whose own docstring calls it "port-agnostic", so it belongs in `services/`; moving it restores the invariant for the LLM half. `simulator.py`→`mock_exchange` is a separate deliberate edge (a simulator needs a fake exchange) and is not covered by the LLM exception at all. **✅ DONE 2026-09-04** (`62b5141`) — and it had to be, because audit finding 14 (the Ollama adapter duplicating the AdvisorRecommendation tail) could NOT be fixed without it: having the adapter call the shared builder would have closed the loop into a real cycle. `extract_last_json_object` moved into `services/llm_cloud_call.py`; `services/` now imports exactly one adapter, the deliberate simulator edge. |
| └ ~~The bidirectional schema-drift guard is live locally and absent from CI~~ *(found 2026-09-04, release-close audit)* | XS | low | ✅ | **✅ RESOLVED 2026-09-04 — but NOT the way this row proposed, and the proposal was wrong.** It said the fix was one `env:` line in `docker-publish.yml`. Measured before writing it: that line would have been **inert**. Every `_report_missing` call site sits behind a `pytest.skip` for a missing operator file; `config/settings.yml` and `.env` are both gitignored (`git ls-files` returns 0) and no workflow creates them, so a fresh CI checkout skips all three regardless of the variable — verified by running the suite with the variable set and no operator files present. Adding it would have produced a guard that reads as armed and checks nothing, one `env:` entry away from the `WOBBLEBOT_REQUIRE_UPGRADE_GATE` comment warning about that exact failure. The check is structurally local-only: the drift it looks for is between an OPERATOR config and the example, and CI has no operator config to drift. **What was actually broken** was that the guard's liveness depended on an untracked `.env` setting the variable — deleted the same day, silently disarming it. Strict mode now defaults ON in tracked code (`WOBBLEBOT_STRICT_CONFIG_DRIFT=0` opts out), pinned by `TestStrictModeIsArmedByDefault` and mutation-verified. |
| └ `test_hide_symbol.py` reaches into a sibling test module for private helpers *(found 2026-09-04, release-close audit)* | XS | low | | It imports `_build_client` and `_make_order` from `tests/web/test_status.py` rather than the shared `tests/web/_helpers.py` the repo built for this. Deliberately left: only three such imports exist repo-wide, one (`test_harvest_command_poll.py`) already carries a comment justifying the same pattern, and moving `_build_client` would churn an 1800-line test file to settle a style point with no correctness cost. Recorded so it is a decision, not an oversight. |
