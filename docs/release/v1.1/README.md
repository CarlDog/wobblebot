...
| └ ~~The capital report's ordermin check says "cannot be sold" about a position that can~~ *(found 2026-09-04)* | S | med | ✅ | **✅ DONE 2026-09-04 — and the operator's first instinct ("use total") was correctly withdrawn mid-implementation.** Reading the call site first showed `available` was a DELIBERATE, incident-derived choice: a comment there records that `total` is a real false-all-clear risk, since a symbol holding most of its position in resting sells would report "sellable" on a free balance that is actually dust. So neither field was wrong — conflating them was. `ExitViability` now carries BOTH, with two named predicates and two sentences: `position_unexitable` (total < ordermin: stranded capital, WARNING + notified) and `free_balance_short` (available < ordermin while the position is fine: INFO, never notified, because it is what a working grid looks like). The ambiguous `blocked` property was REMOVED so no call site can inherit the conflation. `available` stays `None` on the replay path rather than defaulting to total (manufactures an all-clear) or zero (manufactures a finding). Pinned with the live DOGE numbers — 293.64 total, 31.54 available, ordermin 50 — and mutation-verified 4 of 4, including a revert to judging the total question on `available`. |
