...
| **Zero-order layout starvation: back-off / re-park** *(found 2026-08-09, re-anchor e2e)* | S–M | med | ⚠️ | A layout that places 0/N (BUYs refused for reserved USD + SELLs cost-basis-deferred) leaves the symbol onside with no orders → the no-orders self-heal re-lays out **every tick**, silently (refusals log at DEBUG since `e05a931`). Observed live: BTC looped 5s-ly for ~11 min post-re-anchor until operator pause. Fix = starved state + cheap re-attempt condition + one WARN with the refusal breakdown. Detail in `engine.md`. |
| **Web actions: wait-for-completion + auto-refresh** *(operator-filed 2026-08-09)* | M | med-high | ⚠️ | The confirm page stops at "approved"; execution lands a tick later via cli/live's poll and only the 15s dashboard refresh shows it. Fix = the command_result page WATCHES the row (htmx poll → terminal state → show `CommandResult.message`; honest timeout wired to heartbeat data) — never synchronous execution (ADR-002: cli/live stays the only executor). Queue with the per-entity action buttons. Detail in `operator-ux.md`. |
| **Re-anchor viability weighting** *(operator question 2026-08-09)* | M | med | | Weight/annotate banners by whether a re-anchored grid would actually CYCLE (the BTC re-anchor sat idle — drift+age answers "misplaced?", not "worth it?"). **v0 SHIPPED same-day:** the banner's 2h-range-vs-spacing activity stat (fact, not forecast). Full item reuses the P2 screener machinery (band-distance vol, ATR%); annotation only, never auto-suppression. Detail in `operator-ux.md`. |
