...
| Lifetime / cumulative net-PnL number | S | med | | Only `today_realized_pnl()` exists — the cost card has all-time *fees* but no matching all-time *net PnL*. "Have I earned anything, ever?" is the most basic question for a money bot and is currently unanswerable on-screen (data's all on disk; purely a missing aggregation). Sibling of the cost-honesty dashboard; resonates with the operator's mining "am I underwater" framing. |
| Status card: per-order delta column | S | low-med | | Signed-% distance-from-market; template-only. |
| **Symbol-card price for parked/no-order symbols** *(operator-flagged 2026-06-03)* | S | med | | Per-symbol card header shows price + ▼/▲ only when the symbol has open orders; a configured-but-parked symbol (BTC offside) renders a bare `BTC/USD`. Show price+indicator for every configured symbol regardless of order state — data's likely already in `StatusSnapshot.current_prices` (template-ish; confirm parked symbols aren't skipped in the snapshot). Detail in `operator-ux.md`. |
| **Mode-parameterized webui (reuse for live + shadow)** *(operator decision 2026-06-03)* | S–M | med | | Serve the SAME dashboard for both modes; switch the data source by mode (live vs `cli/shadow` ledger). DRY — the UI is already mode-agnostic. **✅ Badge-flip slice DONE 2026-06-03 (v1.1):** `WebConfig.mode` (live\|shadow) → `trading_mode` Jinja global → dynamic `mode-badge` (`_status_card.html`); tests cover both LIVE + SHADOW renders. **Remaining:** data-source / mode-selection plumbing (point the loaders at the shadow ledger; likely a 2nd `cli/web` instance per `cli/up shadow`). Supersedes the "separate shadow page" code comments. Detail in `operator-ux.md`. |
| **Notification level-color inconsistency** *(review 2026-06-03, DEFECT)* | S | med | | `info` renders blue on `notifications.html:39` but green (`ok`) on `history.html:82` — same notification, different color across pages. ~2-line fix (info ≠ success). |
| **Responsive / mobile CSS pass** *(review 2026-06-03)* | M | med | | `base.css` has NO `@media` except `prefers-color-scheme` — 6-7-col tables overflow + navbar crowds on a phone. Horizontal-scroll wrapper on wide tables + nav collapse < ~700px. |
| **Whole-UI design-review punch list** *(review 2026-06-03)* | — | — | | Consolidated frontend-design findings in `operator-ux.md`: dashboard scoreboard strip, per-symbol grid-band sparklines, `/cost` bar chart, fill-flash, advisor collapse, typography/brand elevation, news-coin tags, emergency-confirm weight, CSS-debt cleanups. Tiered defect→elevation. |
