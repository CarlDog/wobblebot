...
| **Command-result echo to Discord** *(found 2026-08-09, re-anchor e2e)* | S | med-high | | Dispatch writes `CommandResult` only to the `pending_commands` row — the operator's ✅ gets silence, which hid a `placed 0/6` re-anchor outcome. Fix = `notify()` in `_process_pending_commands` (message already operator-readable); becomes the renderers item's 8th typed event later, but the plain echo shouldn't wait. Detail in `operator-ux.md`. |
