...
| └ The bidirectional schema-drift guard is live locally and absent from CI *(found 2026-09-04, release-close audit)* | XS | low | | `WOBBLEBOT_STRICT_CONFIG_DRIFT` appears **zero** times under `.github/`. It IS active locally — the repo `.env` sets it and `tests/conftest.py` calls `load_dotenv()` — which is how adding one example key turned the drift test red during the audit. So example/operator drift is caught on a developer machine and NOT on a green CI run. Fix is one `env:` line in `docker-publish.yml`'s pytest step, beside the `WOBBLEBOT_REQUIRE_UPGRADE_GATE: "1"` already there for exactly this reason — that variable's own comment explains the same failure mode (a gate that "degrades to a skip … which reads as green"). The lesson was learned and applied to one of two gates in the same file. **Note the coupling:** turning it on in CI makes any example-file edit fail until the operator's file matches, which is the point, but it will bite the first person who forgets. |
